...
void *recv_function(void *arg)
{
    ...
    int sockfd;
    u16 recvlen,i = 0,tmplen;
    u8 thflag=0;
    u32 ackflag = 0; //TCP_TSOPT|TCP_SACKOPT;
    unsigned char buffer[MAX_PKT_SIZE], buffertmp[MAX_PKT_SIZE];
    ...
    {   
        ...
        if(i==dropno)
        {
            tmplen = rawadvrecv(sockfd, buffertmp, MAX_PKT_SIZE,TCP_SACKOPT|TCP_DISCARD);
            i++;
            continue;
        }    
        //等待接收ACK TCP_SACKOPT
        ...
        //判断是否收到了需要回复ACK的报文
        if(containdata(buffer, recvlen) /*&& !(thflag&TH_FIN)*/ )
        {   
        ...
        if(i==(dropno+5))
        {
            updaterecvstate(buffertmp,tmplen,TCP_SACKOPT);
            adddelaylinktail(53, recvacknumber,ackflag);
...
int main(int argc, char **argv)
{
    ...
    //rawconnect(sockfd);
    rawadvconnect(sockfd, TCP_TSOPT, 0);
    printf("---------conn setup-------------\n");
    //sleep(50);
